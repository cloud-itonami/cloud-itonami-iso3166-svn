(ns marketentry.marketentryllm
  ...
  the SVN public-sector market-entry compliance actor.
  ...
  cited), never a committed record or a real e-JN submission. Every
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world e-JN submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "requires-fdi-notification?=" (:requires-fdi-notification? e)
                        " fdi-notification-verified?=" (:fdi-notification-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-fdi-notification? e))
                              (:fdi-notification-verified? e)))
                   0.9 0.3)}))
