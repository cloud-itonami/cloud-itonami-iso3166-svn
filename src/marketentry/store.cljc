(ns marketentry.store
  "SSoT for the SVN market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems SVN" :portal "e-JN (Elektronsko javno naročanje)"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 700000.0
             :requires-fdi-notification? true :fdi-notification-verified? true
             :drafted? false :submitted? false
             :jurisdiction "SVN" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "e-JN (Elektronsko javno naročanje)"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 700000.0
             :requires-fdi-notification? true :fdi-notification-verified? true
             :drafted? false :submitted? false
             :jurisdiction "ATL" :status :intake}
    "eng-3" {:id "eng-3" :operator "Minami Trading Ljubljana" :portal "e-JN (Elektronsko javno naročanje)"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 999000.0
             :requires-fdi-notification? true :fdi-notification-verified? true
             :drafted? false :submitted? false
             :jurisdiction "SVN" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "e-JN (Elektronsko javno naročanje)"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 700000.0
             :requires-fdi-notification? true :fdi-notification-verified? false
             :drafted? false :submitted? false
             :jurisdiction "SVN" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-fdi-notification? fdi-notification-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-fdi-notification?)    (assoc :engagement/requires-fdi-notification? requires-fdi-notification?)
    (some? fdi-notification-verified?)    (assoc :engagement/fdi-notification-verified? fdi-notification-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-fdi-notification? :engagement/fdi-notification-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-fdi-notification? (boolean (:engagement/requires-fdi-notification? m))
     :fdi-notification-verified? (boolean (:engagement/fdi-notification-verified? m))
