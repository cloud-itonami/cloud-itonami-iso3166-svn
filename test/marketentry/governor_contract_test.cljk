  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems SVN"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems SVN" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest fdi-notification-missing-is-held-and-unoverridable
  (testing "missing verified Slovenian FDI-screening notification for an engagement that requires one -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:fdi-notification-missing} (-> (store/ledger db) last :basis)))
